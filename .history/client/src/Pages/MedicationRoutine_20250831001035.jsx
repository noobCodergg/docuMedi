...
const MedicationRoutine = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const filteredTimes = formData.times.filter(t => t.trim() !== "");
    ...
    try {
      await axios.post("http://localhost:8000/api/personalized/add-medication", {
        ...formData,
        times: filteredTimes
      });
      ...
      <Card className="p-4 shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="grid gap-4 mt-3">
          ...
          {formData.times.map((time, index) => (
            <input
              key={index + "-" + time} // ensure unique key
              type="time"
              name="times"
              value={time}
